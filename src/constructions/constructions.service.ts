...
export class ConstructionsService {
  ...
  async getAvailableConstructions(userId: number) {
    ...
    for (const construction of allConstructions) {
      ...
      // Opción de nueva instancia si cabe
      if (userInstances.length < construction.maxInstances) {
        const level = 1;
        const cost = this.calculateCosts(construction, level);
        const requirements = allRequirements.filter(r => r.targetId === construction.id && r.targetLevel === level);
        const reqMet = this.checkRequirementsMet(requirements, anthill);
        const resMet = this.checkResourcesMet(cost, anthill.resources);
      ...
      // Opción de mejora para cada instancia existente
      for (const instance of userInstances) {
        if (instance.status === ConstructionStatus.COMPLETED) {
            ...
            const requirements = allRequirements.filter(r => r.targetId === construction.id && r.targetLevel === nextLevel);
            const reqMet = this.checkRequirementsMet(requirements, anthill);
            const resMet = this.checkResourcesMet(cost, anthill.resources);
  ...
  async startConstruction(userId: number, constructionId: number, instanceId?: number) {
      ...
      if (!this.checkRequirementsMet(requirements, anthill)) throw new BadRequestException('No se cumplen los requisitos');

      // Verificar recursos
      if (!this.checkResourcesMet(cost, anthill.resources)) throw new BadRequestException('Recursos insuficientes');
  ...
  private checkRequirementsMet(requirements: any[], anthill: any) {
  ...
  private checkResourcesMet(cost: any, userResources: any[]) {
      for (const [resType, amount] of Object.entries(cost)) {
