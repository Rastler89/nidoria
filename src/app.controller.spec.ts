...
// Try to import HelpService if it exists
let HelpService: any;
try { HelpService = require('./help/help.service').HelpService; } catch { HelpService = class {}; }

describe('AppController', () => {
  let appController: AppController;

  const mockAppService = { getHello: jest.fn() };
  const mockAuthService = { login: jest.fn(), register: jest.fn(), refreshAccessToken: jest.fn(), verifyAccount: jest.fn() };
  const mockResourcesService = { getAllResources: jest.fn() };
  const mockColoniesService = { getColonyResources: jest.fn() };
  const mockExpeditionService = { addExpedition: jest.fn() };
  const mockConstructionService = { getUserConstructions: jest.fn(), getAvailableConstructions: jest.fn(), startConstruction: jest.fn() };
  const mockInvestigationService = { getUserInvestigations: jest.fn(), getAvailableInvestigations: jest.fn(), startInvestigation: jest.fn() };
  const mockArmyService = { getArmyStatus: jest.fn(), recruit: jest.fn() };
  const mockHelpService = { getHelp: jest.fn() };
  ...
  beforeEach(async () => {
    const app: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: ConstructionService, useValue: mockConstructionService },
        { provide: InvestigationService, useValue: mockInvestigationService },
        { provide: ArmyService, useValue: mockArmyService },
  ...
  describe('verifyAccount', () => {
    it('should verify an account', async () => {
      ...
      expect(await appController.verifyAccount('1', 'token')).toBe(result);
    });
  });
  ...
  describe('updateMission', () => {
    it('should call expeditionService.addExpedition', async () => {
      const req = { user: { userId: 1 }, body: { resource: 'FOOD', amount: 10 } };
      const result = { id: 1 };
      mockExpeditionService.addExpedition.mockResolvedValue(result);
      expect(await appController.updateMission(req)).toBe(result);
      expect(mockExpeditionService.addExpedition).toHaveBeenCalledWith(1, 'FOOD', 10);
    });
  });
});
